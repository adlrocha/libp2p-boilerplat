...
func initPubSub(t *testing.T, srcStore, dstStore datastore.Batching, stopAt ipld.Link) (legs.LegPublisher, legs.LegSubscriber) {
	srcHost := mkTestHost()
...
func TestRoundTrip(t *testing.T) {
	...
	if _, err := srcStore.Get(datastore.NewKey(lnk2.(cidlink.Link).Cid.String())); err != nil {
		t.Fatalf("data for lnk2 in source store: %v", err)
	}
	if _, err := srcStore.Get(datastore.NewKey(lnk1.(cidlink.Link).Cid.String())); err != nil {
	...
	case downstream := <-watcher:
		...
		if _, err := dstStore.Get(datastore.NewKey(lnk1.(cidlink.Link).Cid.String())); err != nil {
